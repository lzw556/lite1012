...
function useTimeDomain() {
  const [timeDomain, setTimeDomain] = React.useState<TimeDomain>(null!);
  return { timeDomain, setTimeDomain };
}
...
type SubProps = {
  sub: {
    ...
    setTimeDomain: React.Dispatch<React.SetStateAction<TimeDomain>>;
  };
};

export function useSubProps(): SubProps {
  return { sub: { ...useSubProperties(), ...useAxis(), ...useTimeDomain() } };
}
